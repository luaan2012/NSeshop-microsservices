...
const Input = React.forwardRef<HTMLInputElement, IProps>(({ labelText, className, type, ariaInvalid, error, value, disabled, autocomplete, autofocus, inputClassName, ...props }, ref) => {
  return (
    <>
      <div className={`${error ? "animate-shake" : ""} ${className ?? ""} relative dark:text-black`}>
        <input
          ref={ref}
          {...props}
          className={`peer w-full rounded-lg ${error ? "border-red-500" : ""} ${inputClassName ?? ""}`}
          type={type}
        ...
        {error && (
          <small role="alert" className={`${error ? "animate-shake" : ""} text-red-500`}>
            {error}
          </small>
        )}
      </div>
